...
async function renewToken() {
  if (!UserToken) {
    ...
    if (token != null) UserToken = getRenewedToken(token);
  } else UserToken = getRenewedToken(await UserToken);
}
...
async function checkValidity() {
  if (await UserToken) return;
  const token = getCookie("token");

  if (token != null) {
    UserToken = getRenewedToken(token);
  }
  if ((await UserToken) == null) eraseCookie("token");
}
...
export let myGame = new game(
  new paddle(new vec2(-2, -2), new vec2(1, 1)),
  new paddle(new vec2(-2, -2), new vec2(1, 1)),
  new ball(new vec2(-2, -2), new vec2(1, 1), new vec2(0, 0))
);
...
function JSONItirator(form) {
  ...
  for (const key in form) {
    for (const value in form[key]) {
      ...
      } else valuesArray.push(form[key][value]);
    }
  }
  ...
  if (errorElements.length === 1 && form.detail) {
    ...
    return;
  } else if (errorElements.length === 1) return;

  errorElements.forEach((element, index) => {
    if (valuesArray[index]) element.textContent = valuesArray[index];
...
const router = async () => {
  ...
  async function checkForm(form) {
    ...
    if (FullForm) JSONItirator(FullForm);
    if (FullForm.token) return FullForm.token;
    if (FullForm.success) return true;
    return null;
  }
  ...
  async function navigateToOTP(verification, token) {
    ...
    if (otptoken) {
      ...
      if (!sendableCode.method) return 2;
  ...
  renewToken();
  if (match.route.path == "/") {
    if (isLoaded) document.querySelector("#app").style.display = "block";
    ...
    registrationForm.addEventListener("submit", (event) => {
      ...
      UserToken = checkForm(
        view.registerUser(email, username, password1, password2)
      );
      navigateAfterPost(UserToken);
    });
    ...
    profileForm.forEach((form) => {
      form.addEventListener("submit", (event) => {
        ...
        if ("btn-profile-update" == event.submitter.id) {
          FollowingProfile(
            checkForm(view.profileUserPatch(UserToken, username, avatar)),
            true
          );
        } else if (
          event.submitter.id == "accept" ||
          event.submitter.id == "reject"
        ) {
          ...
          const verif = view.profileUserPost(
            UserToken,
            email,
            phone_number,
            otp,
    ...
    if (window.location.protocol === "https:") socketProtocol = "wss://";
    ...
    roomSocket.onmessage = function (e) {
      ...
      if (data.type === "fin du compte") myGame.gameState = "playing";
...
async function displayUser() {
  ...
  if (userElement) {
    ...
    if (
      ...
      !/\/sudoku\/[A-Za-z0-9]{10}\//.test(window.location.pathname)
    ) {
...
async function checkPermissionSudokuLobby(action, actionHandlers) {
  const token = await UserToken;
  if (!token) navigateToInstead("/login/");
...
document.addEventListener("DOMContentLoaded", () => {
  const handleHomePageLoad = () => {
    setTimeout(function () {
      ...
      if (app) document.querySelector("#app").style.display = "none";
      if (loader) document.querySelector(".loader").style.display = "block";
    }, 200);

    setTimeout(function () {
      const app = document.querySelector("#app");
      const loader = document.querySelector(".loader");
      if (loader) document.querySelector(".loader").style.display = "none";
      if (app) document.querySelector("#app").style.display = "block";
      isLoaded = true;
    }, 1200);
  };
  ...
  document.addEventListener("click", function (event) {
    ...
    else if (action) console.warn("No action defined for", action);
  });
    ...
    if (event.target.matches("#disconnect")) {
      ...
      if (window.location.pathname === "/") {
        document.getElementById("user").outerHTML =
          '<a href="/register/" class="navbar-content" id="user" data-link>REGISTER</a>';
      } else {
        navigateTo("/");
      }
