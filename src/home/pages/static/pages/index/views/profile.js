...
export default class extends abstractviews {
  ...
  async getHtml() {
    ...
    const response = await fetch(
      "https://" + DNS + ":8083/auth/profile?request_by=Home",
      options
    );
  ...
  async profileUserPatch(token, username, avatar) {
    ...
    if (response.status === 413) errorElement.innerHTML = "File Too Large";
  ...
  async profileUserPost(token, email, sms, otp, app) {
    ...
    if (app.checked) body["app"] = app.value;
    if (otp && !isEmptyOrWhitespace(otp.value)) body["otp"] = otp.value;
    let response = await fetch(
      "https://" + DNS + ":8083/auth/verification-add?request_by=Home",
      {
        method: "POST",
  ...
  async friendRequest(token, checkbox, fromUser) {
    ...
    if (checkbox) {
      response = await fetch(
        "https://" + DNS + ":8083/auth/send-friend-request?request_by=Home",
        {
          method: "PATCH",
          body: JSON.stringify(body),
          headers: {
  ...
  async deleteFriend(UserToken, friend) {
    ...
    const response = await fetch(
      "https://" + DNS + ":8083/auth/friends?request_by=Home",
      {
        method: "DELETE",
        body: JSON.stringify(body),
        headers: {
