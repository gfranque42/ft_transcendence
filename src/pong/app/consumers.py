...
class PongConsumer(AsyncWebsocketConsumer):
        ...
        try:
            ...
            if len(gTournament.rooms) != 0:
                ...
                if self.room_name in tournamentroom:
                    ...
                    gTournament.rooms[
                        self.room_name
                    ].roomGroupName = self.room_group_name
            ...
            if self.room_name == gTournament.lobbyRoom.roomName:
                ...
                if gTournament.lobbyRoom.channelLayer == None:
                    ...
                    print(
                        self.room_name,
                        ": channel layer type: ",
                        gTournament.lobbyRoom.channelLayer,
                        flush=True,
                    )
                ...
                print(
                    self.room_name,
                    ": partyType = [",
                    type(gRoomsManager.rooms[self.room_name].partyType),
                    "]",
                    flush=True,
                )
        ...
        if self.tournament == True:
            ...
            if self.username:
                ...
                await self.channel_layer.group_send(
                    self.room_group_name,
                    {"type": "quit", "message": "quitting", "left": self.username},
                )
        else:
            if (
                gRoomsManager.rooms[self.room_name].partyType != 4
                and gRoomsManager.rooms[self.room_name].ready == False
            ):
                try:
                    await gRoomsManager.rooms[self.room_name].removePlayer(
                        self.username
                    )
                except Exception as e:
                    print(self.room_name, ": error: ", e, flush=True)
            ...
            if (
                gRoomsManager.rooms[self.room_name].thread
                and self.username
                and gRoomsManager.rooms[self.room_name].players[0] == self.username
                and gRoomsManager.rooms[self.room_name].finish == True
            ):
        ...
        if type_data == "username":
            ...
            if self.tournament == True:
                ...
                if gTournament.rooms[self.room_name].ready == True:
                    await self.channel_layer.group_send(
                        self.room_group_name,
                        {"type": "gameUpdate", "message": "ready for playing"},
                    )
                ...
                if gRoomsManager.rooms[self.room_name].partyType != 4:
                    try:
                        await gRoomsManager.rooms[self.room_name].addPlayer(
                            self.username
                        )
                        if (
                            gRoomsManager.rooms[self.room_name].partyType > 0
                            and gRoomsManager.rooms[self.room_name].partyType < 4
                        ):
                            await gRoomsManager.rooms[self.room_name].addPlayer("AI")
                            gRoomsManager.rooms[
                                self.room_name
                            ].paddleR.ai = gRoomsManager.rooms[self.room_name].partyType
                    except Exception as e:
                        print(self.username, ": error: ", e, flush=True)
                        await self.close()
            ...
            if self.tournamentRoom == True:
                gTournament.rooms[self.room_name].updateData(
                    text_data_json, self.username
                )
            else:
                gRoomsManager.rooms[self.room_name].updateData(
                    text_data_json, self.username
                )
    ...
    async def gameUpdate(self, event):
        ...
        if message == "update":
            print(
                self.username, ": valeur de paddledir:", event["paddleLd"], flush=True
            )
            ...
            if self.tournament == False and self.tournamentRoom == False:
                await self.send(
                    text_data=json.dumps(
                        {
                            "type": "ready for playing",
                            "message": "ready for playing",
                            "player1Name": gRoomsManager.rooms[self.room_name].players[
                                0
                            ],
                            "player2Name": gRoomsManager.rooms[self.room_name].players[
                                1
                            ],
                        }
                    )
    ...
    async def tournamentRedirect(self, event):
        ...
        print(
            "From Tournament: ", event["numberOfRooms"], " number of rooms", flush=True
        )
        roomKey = ""
    ...
    async def quit(self, event):
        ...
        if (
            self.tournamentRoom == True
            and gTournament.rooms[self.room_name].inGame == True
        ):
