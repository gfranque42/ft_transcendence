...
class room:
        ...
        if self.partyType < 4:
            ...
            with self.lock:
                if player in self.players:
                    ...
                    raise roomException(
                        self.roomName + ": Player " + player + " isn't in this room!",
                        1002,
                    )
    ...
    async def countDown(self) -> None:
        ...
        if self.ready == True and self.inGame == False:
            ...
            raise roomException(
                "Room "
                + self.roomName
                + " haven't the good amount of players or is already in game",
                1003,
            )
    ...
    def gameLoop(self) -> None:
        ...
        while self.inGame == True:
            # with self.lock:
            self.paddleL, self.paddleR, self.ball, self.scoreL, self.scoreR = (
                gameUpdate(
                    self.paddleL, self.paddleR, self.ball, self.scoreL, self.scoreR
                )
            )
    ...
    def sendUpdate(self, message) -> None:
        ...
        if obj["message"] == "finish":
            async_to_sync(self.channelLayer.group_send)(self.roomGroupName, obj)
        else:
            keys = {
                "type",
                "message",
...
class tournament:
    ...
    async def sendRooms(self) -> None:
        ...
        try:
            print(
                "channelLayer: ",
                self.lobbyRoom.channelLayer,
                ", roomgroupname: ",
                self.lobbyRoom.roomGroupName,
                flush=True,
            )
            await self.consumer.channel_layer.group_send(
                self.consumer.room_group_name, infos
            )
    ...
    def routine(self) -> None:
        ...
        print(
            self.lobbyRoom.roomName, ": liste des joueurs: ", self.players, flush=True
        )
        while len(self.players) > 1:
            print(
                self.lobbyRoom.roomName,
                ": liste des joueurs: ",
                self.players,
                flush=True,
            )
